...
while epis_count < max_frame:
    ...
    while not done:
        ...
        if sample < effective_eps:
            ...
            data = [nd.array(state.reshape([1, frame_len * channel, image_size, image_size]), ctx),
                    nd.array([100], ctx)]
        ...
        if done:
            if epis_count % 50. == 0.:
                print('epis[%d],eps[%.4f],durat[%d],fnum=%d, cum_cl_rew = %d, cum_rew = %.4f,tot_cl = %.4f , tot = %.4f' \
                      % (epis_count, eps, t + 1, frame_counter, cum_clipped_reward, cum_reward, moving_average_clipped,
                         moving_average))
    # epis_count += 1
