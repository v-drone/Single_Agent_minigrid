...
class MobileNet(TorchModelV2, torch.nn.Module):
    ...
    def __init__(self, obs_space, action_space: Space, num_outputs, model_config, name):
        ...
        self.conv_layers.append(torch.nn.Conv2d(3, 8, kernel_size=1, stride=1, padding=0))
        self.conv_layers.append(MobileNetV3(inverted_residual_setting, last_channel, num_classes=3).features[1: -1])
        self.conv_layers.append(torch.nn.AdaptiveAvgPool2d(1))
        self.conv_layers.append(torch.nn.Flatten(start_dim=1))
        ...
        self.fc_layers = torch.nn.Sequential(
            SlimFC(feature_in, 256, activation_fn='relu'),
