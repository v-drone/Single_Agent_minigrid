...
class DroneConnector(object):
    ...
    def do_action(self, action):
        if action == 0:
            ...
            self.client.moveByVelocityAsync(self.speed, 0, 0, 1, vehicle_name="").join()
        elif action == 1:
            # Move forward at medium speed
            self.client.moveByVelocityAsync(self.speed * 2, 0, 0, 1, vehicle_name="").join()
        elif action == 2:
            # Move forward at high speed
            self.client.moveByVelocityAsync(self.speed * 3, 0, 0, 1, vehicle_name="").join()
        elif 3 <= action <= 4:
            # Rotate in place at various yaw rates
            yaw_rate = 45 * (action - 2)  # Degrees per second
            self.client.rotateByYawRateAsync(yaw_rate, 1, vehicle_name="").join()
        elif 5 <= action <= 6:
            # Rotate in place at various yaw rates
