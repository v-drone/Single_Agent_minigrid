...
class AddBatteryWrapper(ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        energy_bar_width = int(energy_fraction * obs.shape[1])

        energy_bar_img = np.ones((ENERGY_BAR_HEIGHT, obs.shape[1], 3), dtype=np.uint8) * 255
        ...
        observation = np.concatenate((energy_bar_img, obs[cut_off[0]:-cut_off[1], :, :]), axis=0)
        return observation
